/**
 * manageIndex.js  卡片管理页面入口js
 * 功能：卡片管理页功能入口，获取datajs和调用具体功能
 */

var APP_DATA   = null ,  // 后台向前台传递的datajs中的数据
    datajs_bak = null ,  // 后台向前台传递的datajs中的数据--BAK 用于对卡片操作后传递给客户端
    BASE_URL   = ""   ,  // 路径
    SID        = ""   ,  // cookie
...
function initParameter () {
    // 1.获取当前用户cookie，方便接下来对卡片的删除或增加操作
    var requestParam = {
    ...
    CLIENT.getCookieByClient(requestParam);
...
function setAppData (result) {
    // 备份返回的数据，在切换卡片状态的时候，对它进行修改（会重新把datajs组织好，并且返给客户端，客户端直接保存）
    datajs_bak = result;
    if (result) {
        // 获取动态manager服务器地址
